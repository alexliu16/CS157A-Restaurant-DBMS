		...
		applyButton.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if(binder.validate().isOk()) { //All fields are filled in and formatted correctly
					...
					Notification.show("Successfully update billing information");
					Customer cust = getCustomer();
					getRestaurantDAO().updateCustomerCC(cust.getUsername(), cust.getCreditCardNumber(), Long.parseLong(ccNoField.getValue()), expDateField.getValue(), Integer.parseInt(cvvField.getValue()));
				}	
				else
