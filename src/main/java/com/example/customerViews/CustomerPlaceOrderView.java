...
public class CustomerPlaceOrderView extends CustomerMainView {
	...
	public CustomerPlaceOrderView(Navigator navigate, RestaurantDAO rDAO) {
		super(navigate, rDAO);
	...
	private void initializeOrderPanel() {
		...
		// Add header
		Label headerLabel = new Label("My Order:");
		...
		orderButton.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if(orderItems.isEmpty()) //no items in order
					...
					navigator.navigateTo("CustomerConfirmOrderView");
					((CustomerConfirmOrderView) navigator.getCurrentView()).setOrderItems(orderItems);
					((CustomerConfirmOrderView) navigator.getCurrentView()).setCustomer(getCustomer());
					//reset order
					orderItems = new TreeMap<>();
					updateOrder();
				}
		...
		panelContent.addComponents(itemsLayout, subtotalLayout, taxLayout, totalLayout, orderButton);
